...
class Addcorporate {
  async addcorporate(req, res) {
    ...
    try {
      ...
      if (lunchSlots) {
        try {
          parsedLunchSlots = typeof lunchSlots === 'string' ? JSON.parse(lunchSlots) : lunchSlots;
        } catch (error) {
      ...
      if (dinnerSlots) {
        try {
          parsedDinnerSlots = typeof dinnerSlots === 'string' ? JSON.parse(dinnerSlots) : dinnerSlots;
        } catch (error) {
  ...
  async updatecorporatelist(req, res) {
    ...
    if (apartmentdelivaryprice) obj["apartmentdelivaryprice"] = apartmentdelivaryprice;
    ...
    if (lunchSlots) {
      try {
        obj["lunchSlots"] = typeof lunchSlots === 'string' ? JSON.parse(lunchSlots) : lunchSlots;
      } catch (error) {
        console.log('Error parsing lunch slots:', error);
        obj["lunchSlots"] = [];
      }
    ...
    if (dinnerSlots) {
      try {
        obj["dinnerSlots"] = typeof dinnerSlots === 'string' ? JSON.parse(dinnerSlots) : dinnerSlots;
      } catch (error) {
        console.log('Error parsing dinner slots:', error);
        obj["dinnerSlots"] = [];
      }
