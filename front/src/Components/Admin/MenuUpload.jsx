...
const MenuUpload = () => {
  ...
  // This function saves the *current product's* assignments to the DB
  const handleSaveProductAssignments = async () => {
    ...
    const currentAssignmentsForProduct = assignments[currentProductId];

    if (!currentAssignmentsForProduct) {
      toast.error("No assignments found for this product.");
      return false;
    }
    ...
    const payload = {
      ...
      // 7. FIX: Use your field 'foodprice' as the basePrice
      basePrice: currentProduct.foodprice,
      hubData: Object.values(currentAssignmentsForProduct), // Send all hub settings for this product
    };

    try {
      // Use the 'create' route we defined in the backend plan
      await axios.post(`${SAVE_API_URL}/create`, payload);
      // 8. FIX: Use 'foodname' in toast
      toast.success(`Saved assignments for ${currentProduct.foodname}`);
      setLoading(false);
